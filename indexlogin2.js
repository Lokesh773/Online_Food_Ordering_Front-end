...
function signupValid(){
	if ((firstname.value.length <= 4) || (firstname.keyCode > 48 && firstname.keyCode < 57)) {
		signup_error.style.display = "block";
		firstname.style.border = "1px solid red";
		signup_error.innerText = "valid Firstname";
		firstname.focus();
		return false;
	}
	...
	if (email.value.length ==''||email.value.indexOf('@')== -1 ||email.value.indexOf('.')== -1 ) {
		...
		signup_error.innerText = "Please Enter Valid Email";
		email.focus();
		return false;
	}
